...
You also have a more powerful version of +can+, called +enforce+, which takes a
block and will raise +HAL::ICantLetYouDoThat+ if the block evaluates to false:

    HAL.enforce(dave) {|can| can.open_the_pod_bay_doors? } #=> raises!

Finally, you can include +HAL+ into your classes to get +can+ and +enforce+ as
helper methods.
    ...
    class UsersController < ApplicationController
      ...
      def check_permissions
        enforce(current_user) {|can| can.edit_user?(current_user) }
      end
    end
